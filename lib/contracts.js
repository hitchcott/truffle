...
var Contracts = {
  ...
  provision: function(options, fetch_accounts, callback) {
    ...
    function getImports(cb) {
      if (!options.imports) { return cb(); }
      const imports = options.imports.map((filename) => {
        // split it at /build/
        const [start, end] = filename.split('/build/contracts/');
        let newImport;
        // try to resolve local one
        try {
          newImport = process.env.PWD+'/build/contracts/'+end;
        } catch(e) {
          newImport = filename;
        };
        return newImport;
    ...
    getSources(function(sources) {
      getImports(function(imports) {
        getAccounts(function(err, accounts) {
          ...
          var contracts = sources.concat(imports || []);
